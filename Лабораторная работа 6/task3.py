...
def to_cvd_file(filename, header, rows, delimiter=',', new_line='\n'):
    with open(filename, 'wt', encoding='utf-8') as f:
        f.write(delimiter.join(header)+new_line)
        for i in rows: f.write(delimiter.join(i)+new_line)
...
to_cvd_file(OUTPUT_FILE, headers_list, data)
